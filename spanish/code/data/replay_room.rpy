translate spanish strings:

    # game/code/data/replay_room.rpy:15
    old "Opening Cutscene"
    new "Escena de apertura"

    # game/code/data/replay_room.rpy:15
    old "Training Cutscene"
    new "Escena de entrenamiento"

    # game/code/data/replay_room.rpy:15
    old "Phoebe Dream"
    new "El sueño de Phoebe"

translate spanish strings:

    # game/code/data/replay_room.rpy:16
    old "C01: Opening Cutscene"
    new "C01: Escena de apertura"

    # game/code/data/replay_room.rpy:16
    old "C02: Training Cutscene"
    new "C02: Escena de entrenamiento"

    # game/code/data/replay_room.rpy:16
    old "C04: Phoebe Dream"
    new "C04: El sueño de Phoebe"

    # game/code/data/replay_room.rpy:16
    old "C04: Delphia Masturbating"
    new "C04: Delfia Masturbándose"

    # game/code/data/replay_room.rpy:16
    old "C04: Painting Delphia"
    new "C04: Delphia Pintada"

    # game/code/data/replay_room.rpy:16
    old "C04: Locker Room Fling With Iona"
    new "C04: Aventura con Iona en los vestuarios"

    # game/code/data/replay_room.rpy:16
    old "You like partying."
    new "Te gusta la fiesta"

    # game/code/data/replay_room.rpy:16
    old "C04: Favor From Nirmala"
    new "C04: El favor de Nirmala"

    # game/code/data/replay_room.rpy:16
    old "Redeem the favor."
    new "Canjea el favor"

    # game/code/data/replay_room.rpy:16
    old "C05: Josie Sleepwalk"
    new "C05: Josie Sonámbula"

    # game/code/data/replay_room.rpy:16
    old "C05: Private Lesson With Ronja"
    new "C05: Lección privada con Ronja"

    # game/code/data/replay_room.rpy:16
    old "C06: Ronja Booty Call"
    new "C06: Llamada de culo de Ronja"

    # game/code/data/replay_room.rpy:16
    old "Call Ronja."
    new "Llama a Ronja."

    # game/code/data/replay_room.rpy:16
    old "C06: Free Bone From Delphia"
    new "C06: Hueso libre de Delphia"

    # game/code/data/replay_room.rpy:16
    old "Ask the right questions."
    new "Haga las preguntas correctas."

    # game/code/data/replay_room.rpy:16
    old "C06: Worshipping Sally"
    new "C06: Adorando a Sally"

    # game/code/data/replay_room.rpy:16
    old "Stay with Sally."
    new "Quédate con Sally."

    # game/code/data/replay_room.rpy:16
    old "C06: Handjob From Fleur"
    new "C06: Paja De Fleur"

    # game/code/data/replay_room.rpy:16
    old "Let Fleur continue."
    new "Deja que Fleur continúe."

    # game/code/data/replay_room.rpy:16
    old "C06: Pool Game With Nirmala"
    new "C06: Juego de billar con Nirmala"

    # game/code/data/replay_room.rpy:16
    old "C06: Finish What Fleur Started"
    new "C06: Termina lo que empezó Fleur"

    # game/code/data/replay_room.rpy:16
    old "Go with Fleur."
    new "Ve con Fleur."

    # game/code/data/replay_room.rpy:16
    old "C06: Help From Iona"
    new "C06: Ayuda de Iona"

    # game/code/data/replay_room.rpy:16
    old "Go with Iona."
    new "Ve con Iona."

    # game/code/data/replay_room.rpy:16
    old "C07: Diana Jumps You"
    new "C07: Diana salta encima de ti"

    # game/code/data/replay_room.rpy:16
    old "Don't rebuff Diana."
    new "No rechaces a Diana."

    # game/code/data/replay_room.rpy:16
    old "C07: Sally's Hidden Entrance"
    new "C07: La entrada oculta de Sally"

    # game/code/data/replay_room.rpy:16
    old "Reward Sally."
    new "Recompensa a Sally."

    # game/code/data/replay_room.rpy:16
    old "C07: Bonding With Nia"
    new "C07: Lazos con Nia"

    # game/code/data/replay_room.rpy:16
    old "C07: Getting Better Acquainted"
    new "C07: Familiarizarse mejor"

    # game/code/data/replay_room.rpy:16
    old "C07: Finish What Delphia Started"
    new "C07: Termina lo que Delphia empezó"

    # game/code/data/replay_room.rpy:16
    old "Help Delphia."
    new "Ayuda a Delphia."

    # game/code/data/replay_room.rpy:16
    old "C07: Seduce Irene"
    new "C07: Seduce a Irene"

    # game/code/data/replay_room.rpy:16
    old "Go all the way."
    new "Ve hasta el final."

    # game/code/data/replay_room.rpy:16
    old "C07: An Artist's Head"
    new "C07: La cabeza de un artista"

    # game/code/data/replay_room.rpy:16
    old "Investigate the artist."
    new "Investiga a la artista."

    # game/code/data/replay_room.rpy:16
    old "C07: Karaoke With Nirmala"
    new "C07: Karaoke con Nirmala"

    # game/code/data/replay_room.rpy:16
    old "C07: Reward From Corinne"
    new "C07: Recompensa de Corinne"

    # game/code/data/replay_room.rpy:16
    old "Investigate Corinne."
    new "Investiga a Corinne."

    # game/code/data/replay_room.rpy:16
    old "C07: Fun Time With Iona"
    new "C07: Tiempo de diversión con Iona"

    # game/code/data/replay_room.rpy:16
    old "Stop Iona from serving."
    new "Evita que Iona sirva."

    # game/code/data/replay_room.rpy:16
    old "C07: Skinny Dipping With Isa"
    new "C07: Baño desnudo con Isa"

    # game/code/data/replay_room.rpy:16
    old "Investigate Isa."
    new "Investiga a Isa."

    # game/code/data/replay_room.rpy:16
    old "C07: Reward From Sam"
    new "C07: Recompensa de Sam"

    # game/code/data/replay_room.rpy:16
    old "Investigate Sam."
    new "Investiga a Sam."

    # game/code/data/replay_room.rpy:16
    old "C08: Ivy's Welcoming Party"
    new "C08: Fiesta de bienvenida de Ivy"

    # game/code/data/replay_room.rpy:16
    old "C08: Loving Lumi"
    new "C08: Amando a Lumi"

    # game/code/data/replay_room.rpy:16
    old "Comfort Lumi."
    new "Consuela a Lumi."

    # game/code/data/replay_room.rpy:16
    old "C08: Ritual In The Cave"
    new "C08: Ritual En La Cueva"

    # game/code/data/replay_room.rpy:16
    old "C08: Spiritual Awakening With Nia"
    new "C08: El despertar espiritual con Nia"

    # game/code/data/replay_room.rpy:16
    old "C08: Lumi's Punishment"
    new "C08: El castigo de Lumi"

    # game/code/data/replay_room.rpy:16
    old "C08: Self-Help Group Orgy"
    new "C08: Orgía del grupo de autoayuda"

    # game/code/data/replay_room.rpy:16
    old "C08: Breeding Ivy"
    new "C08: Hiedra reproductora"

    # game/code/data/replay_room.rpy:16
    old "C08: Date with Zemfira"
    new "C08: Cita con Zemfira"

    # game/code/data/replay_room.rpy:16
    old "C08: Comforting Asami"
    new "C08: Consolando a Asami"

    # game/code/data/replay_room.rpy:16
    old "Help out Asami."
    new "Ayuda a Asami."

    # game/code/data/replay_room.rpy:16
    old "C08: Hellfire Hellena"
    new "C08: Hellena del Fuego Infernal"

    # game/code/data/replay_room.rpy:16
    old "You don't mind at all."
    new "No te importa en absoluto."

    # game/code/data/replay_room.rpy:16
    old "C08: In The Boss's Bed"
    new "C08: En la cama de la jefa"

    # game/code/data/replay_room.rpy:16
    old "C08: Taking Care Of Zemfira"
    new "C08: Cuidando a Zemfira"

    # game/code/data/replay_room.rpy:16
    old "C08: Licking Morgan's Wounds"
    new "C08: Lamiendo las heridas de Morgan"

    # game/code/data/replay_room.rpy:16
    old "Ask Morgan to stay."
    new "Pídele a Morgan que se quede."

    # game/code/data/replay_room.rpy:16
    old "C08: Extra Practice With Iona"
    new "C08: Práctica adicional con Iona"

    # game/code/data/replay_room.rpy:16
    old "Encourage Iona."
    new "Anima a Iona."

    # game/code/data/replay_room.rpy:16
    old "C08: Holding Moans With Monika"
    new "C08: Gimiendo con Monika"

    # game/code/data/replay_room.rpy:16
    old "Give into the temptations."
    new "Cede a las tentaciones."

    # game/code/data/replay_room.rpy:16
    old "C08: Healed By The Enemy"
    new "C08: Sanado por el enemigo"

    # game/code/data/replay_room.rpy:16
    old "Seize the moment."
    new "Aprovecha el momento."

    # game/code/data/replay_room.rpy:16
    old "C08: Comforting Silvia"
    new "C08: Consolando a Silvia"

    # game/code/data/replay_room.rpy:16
    old "C09: Josie's First Time"
    new "C09: Pasar tiempo con Josie"

    # game/code/data/replay_room.rpy:16
    old "C09: Josie's First Time (Trans)"
    new "C09: Pasar tiempo con Josie (Trans)"

    # game/code/data/replay_room.rpy:16
    old "C09: Satisfying Ronja"
    new "C09: Satisfaciendo a Ronja"

    # game/code/data/replay_room.rpy:16
    old "C09: Penetrating Diana"
    new "C09: Penetrando a Diana"

    # game/code/data/replay_room.rpy:16
    old "Don't Stop Diana"
    new "No detengas a Diana"

    # game/code/data/replay_room.rpy:16
    old "C09: Exploring With Irene"
    new "C09: Explorando con Irene"

    # game/code/data/replay_room.rpy:16
    old "Investigate the Cult."
    new "Investiga el culto."

    # game/code/data/replay_room.rpy:16
    old "C09: Helping Isa"
    new "C09: Ayudando a Isa"

    # game/code/data/replay_room.rpy:16
    old "Investigate the Winners Club."
    new "Investiga el Club de los Ganadores."

    # game/code/data/replay_room.rpy:16
    old "C09: Flirting With Foxy & Lilith"
    new "C09: Coqueteando con Foxy y Lilith"

    # game/code/data/replay_room.rpy:16
    old "Investigate the Syndicate."
    new "Investiga el Sindicato."

    # game/code/data/replay_room.rpy:16
    old "C09: Zemfira's Temptations"
    new "C09: Las tentaciones de Zemfira"

    # game/code/data/replay_room.rpy:16
    old "Insult Xasan."
    new "Insulta a Xasan."

    # game/code/data/replay_room.rpy:16
    old "C09: Winner's Strip Tease"
    new "C09: Strip Tease del ganador"

    # game/code/data/replay_room.rpy:16
    old "C09: Naked Mile With Iona & Isa"
    new "C09: Milla al desnudo con Iona e Isa"

    # game/code/data/replay_room.rpy:16
    old "C09: Rope Play With Delphia"
    new "C09: Juego de cuerdas con Delphia"

    # game/code/data/replay_room.rpy:16
    old "C09: Blowjob From Nia"
    new "C09: Mamada De Nia"

    # game/code/data/replay_room.rpy:16
    old "C09: Dominating Fleur"
    new "C09: Fleur dominante"

    # game/code/data/replay_room.rpy:16
    old "C09: Nia & Silvia Threesome"
    new "C09: Trío con Nia y Silvia"

    # game/code/data/replay_room.rpy:16
    old "Investigate respective groups."
    new "Investigar los respectivos grupos."

    # game/code/data/replay_room.rpy:16
    old "C09: Silvia & Zemfira Threesome"
    new "C09: Trío con Silvia y Zemfira"

    # game/code/data/replay_room.rpy:16
    old "C09: Zemfira & Nia Threesome"
    new "C09: Trío con Zemfira y Nia"

